...
- 云同步：结构化记录（菜谱、外出就餐等）在 Cloudflare D1 数据库 `shiji-sync`，配图字节在 R2 `shiji-images`（内容哈希）。**禁止把图片二进制或 data URL 写入 D1**。旧 KV 快照只读回退；本机仍用 localStorage `shiji-state-v2`。上传以本机为准（含删除）；下载以云端为准。
