...
import{S,update,uid,today,addDays,monthOf,fmtDate,fmtTime,MEALS,toast,dayIntake,dayEntries,addManualEntry,removeManualEntry,setEntryCalories,monthDates,expiringItems,nameMatch,daysUntil,diningVenues,diningCatsOf}from'./store.js';
...
const venueLabel=d=>d.venue==='外卖'&&d.dineIn?'外卖 · 可堂食':(d.venue||'外卖');
const card=d=>`<div class="dining-card">${d.image?`<div class="dining-thumb"><img class="zoomable" src="${esc(imgSrc(d.image))}" alt="${esc(d.name)}" title="点击放大查看"></div>`:''}<div class="daily-top"><strong>${esc(d.name)}</strong>${d.calories?`<span class="badge fresh">${d.calories} 千卡</span>`:''}</div><span class="food-meta"><em class="venue-tag">${esc(venueLabel(d))}</em> ${esc([d.place,d.category].filter(Boolean).join(' · '))}</span><div class="daily-actions"><button class="text-button" data-menu="${d.id}">${ico('plus',13)} 点菜</button><button class="icon-button" data-edit="${d.id}" aria-label="编辑">${ico('edit',15)}</button><button class="icon-button" data-del="${d.id}" aria-label="删除">${ico('trash',15)}</button></div></div>`;
const listed=()=>S.dining.filter(d=>(renderDining.venue==='全部'||d.venue===renderDining.venue)&&(renderDining.cat==='全部'||d.category===renderDining.cat)&&(d.name+' '+(d.place||'')+' '+d.category+' '+(d.venue||'')).toLowerCase().includes(q));
...
dlg.innerHTML=`<form class="editor" id="dining-form"><div class="modal-heading"><div><span class="eyebrow">EATING OUT</span><h2>${isNew?'新增外出餐':'编辑外出餐'}</h2></div><button type="button" class="icon-button" data-close aria-label="关闭">${ico('close')}</button></div><div class="editor-content"><label class="field">菜品名称 <span class="required">*</span><input name="name" required maxlength="60" placeholder="例如：番茄牛腩面" value="${esc(d?.name||'')}"></label><div class="field-row"><label class="field">大分类<select name="venue">${venues.map(v=>`<option ${(d?.venue||'外卖')===v?'selected':''}>${esc(v)}</option>`).join('')}</select></label><label class="field">小分类<select name="category"></select></label></div><label class="prep-line" id="dine-in-line"><input type="checkbox" name="dineIn" ${d?.dineIn?'checked':''}> 可堂食（外卖适用）</label><div class="field-row"><label class="field">店名 / 品牌<input name="place" maxlength="40" placeholder="例如：楼下面馆" value="${esc(d?.place||'')}"></label><label class="field">热量（千卡）<input name="calories" type="number" min="0" max="9999" value="${d?.calories??''}"></label></div><label class="field">备注 <span class="optional">只在编辑里可见</span><input name="description" maxlength="120" placeholder="口味、地址、常点搭配…" value="${esc(d?.description||'')}"></label></div><div class="modal-footer"><span></span><div><button type="button" class="secondary" data-close>取消</button><button type="submit" class="primary">${ico('check')} 保存</button></div></div></form>`;
if(!dlg.open)dlg.showModal();
dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>dlg.close());
const syncVenue=()=>{const venue=dlg.querySelector('[name=venue]')?.value||'外卖';const line=dlg.querySelector('#dine-in-line');if(line)line.hidden=venue!=='外卖';fillCats(venue,venue===(d?.venue)?d?.category:'')};
dlg.querySelector('[name=venue]')?.addEventListener('change',syncVenue);syncVenue();
(dlg.querySelector('#dining-form')??document.createElement('button')).onsubmit=e=>{e.preventDefault();const f=new FormData(e.target);const venue=f.get('venue')||'外卖';const next={id:isNew?uid():d.id,name:f.get('name').trim(),place:f.get('place').trim(),venue,dineIn:venue==='外卖'&&f.get('dineIn')==='on',category:f.get('category'),calories:f.get('calories')?Number(f.get('calories')):null,hours:d?.hours||0,minutes:d?.minutes||0,servings:d?.servings||1,description:f.get('description').trim(),image:d?.image||''};if(!next.name){toast('请填写名称');return}if(!update(()=>{if(isNew)S.dining.unshift(next);else S.dining=S.dining.map(x=>x.id===next.id?next:x)}))return;gcImages(S);dlg.close();toast('已保存')}}
