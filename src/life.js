...
// —— 就餐记录 ——
const logLine=e=>{const spec=e.specs&&Object.keys(e.specs).length?' · '+Object.entries(e.specs).map(([k,v])=>v).join('/'):'';const note=e.note?' · '+e.note:'';return`${e.name}${e.qty>1?` ×${e.qty}`:''}${spec}${note}`};
...
mount.innerHTML=`<section class="intro"><div><div class="eyebrow">DAILY ENERGY</div><h1>今天的热量，心里有数。</h1><p>本周菜单勾选「已吃」会记入；每条热量都可以改，就餐记录会跟着同步。</p></div></section><div class="health-goal"><label class="field">每日目标（千卡）<input id="goal" type="number" min="0" max="20000" value="${goal}"></label></div><div class="stat-cards"><div class="stat"><span class="stat-label">今日已摄入</span><strong>${eaten}<small> 千卡</small></strong><div class="stat-bar"><i style="width:${pct}%"></i></div></div><div class="stat"><span class="stat-label">目标剩余</span><strong>${remain}<small> 千卡</small></strong><span class="stat-sub">${goal?`${pct}% / 目标 ${goal} 千卡`:'先设定每日热量目标'}</span></div><div class="stat"><span class="stat-label">今日记录</span><strong>${todayLines.length}<small> 餐</small></strong><span class="stat-sub">${todayLines.map(logLine).slice(0,2).join('、')||'还没吃'}</span></div></div><div class="collection-bar"><div class="collection-title"><h2>每日摄入</h2><span>本月 · 目标 ${goal} 千卡</span></div></div><div class="journal-list">${days.map(d=>`<div class="journal-day"><header><strong>${fmtDate(d)} ${d===t?'· 今天':''}</strong><span>${dayIntake(d)} / ${goal} 千卡 ${goal?`(${Math.min(999,Math.round(dayIntake(d)/goal*100))}%)`:''}</span></header><div class="day-bar"><i style="width:${goal?Math.min(100,dayIntake(d)/goal*100):0}%"></i></div>${dayEntries(d).map(e=>`<div class="journal-item health-line"><em>${MEALS.find(m=>m[0]===e.meal)?.[1]||''}</em><span class="health-name">${esc(logLine(e))}</span><label class="cal-edit-wrap"><input class="cal-edit" data-cal="${e.id?`m|${d}|${e.id}`:`w|${d}|${e.meal}|${e.idx}`}" type="number" min="0" max="99999" value="${e.calories||0}"><small>千卡</small></label>${e.id?`<button class="icon-button" data-mdel="${d}|${e.id}" aria-label="删除手动记录">${ico('close',12)}</button>`:''}</div>`).join('')}</div>`).join('')||`<div class="empty"><div>${ico('flame')}</div><h3>本月还没有摄入记录</h3><p>在本周菜单勾选「已吃」，或在上方手动添加。</p></div>`}</div>`;
