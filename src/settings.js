...
list.querySelector('[data-add]').onclick=()=>{const inp=list.querySelector('.cat-add input');const nv=inp.value.trim();if(!nv)return;if(S.cats[key].includes(nv)){toast('分类已存在');return}update(()=>S.cats[key].push(nv));openSettings(onDone)}});
