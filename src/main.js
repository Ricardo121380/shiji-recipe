...
dlg.querySelector('#bind-go')?.addEventListener('click',()=>{try{sync.bindCode(dlg.querySelector('#sync-code').value)}catch(e){toast(e.message);return}dlg.close();toast('同步码已绑定，开始首次同步');sync.tick(true).then(()=>renderApp())});
dlg.querySelector('#sync-now')?.addEventListener('click',()=>{dlg.close();sync.tick(true).then(()=>renderApp())});
