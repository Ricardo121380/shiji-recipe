...
else{dlg.innerHTML=`<div class="editor"><div class="modal-heading"><div><span class="eyebrow">CLOUD SYNC</span><h2>云端同步</h2></div><button class="icon-button" data-close aria-label="关闭">${ico('close')}</button></div><div class="editor-content"><p style="margin-top:0">同步码：<strong>······${sync.getCode().slice(-4)}</strong>（完整码仅存于各设备浏览器）</p><p>本机：<strong>${S.recipes.length}</strong> 道菜谱 · 云端：<strong>${sync.lastCloudRecipes()}</strong> 道菜谱<br>云端最后更新：${sync.lastCloudAt()||'尚无'}<br>本地上次同步：${sync.lastSyncAt()||'尚无'}</p><p class="muted">${esc(sync.lastPlanHint()||'正在查看云端状态…')}</p><label class="prep-line"><input type="checkbox" id="sync-pause" ${sync.isPaused()?'checked':''}> 暂停自动同步（改动只保留在本机）</label><p class="muted">「立即同步」不是只上传：会先看云端和本机谁更新，覆盖前会再问一次。下载会整份替换本机，上传会整份替换云端，两边的菜谱不会自动合并。自动同步：打开网站时、改动后约 4 秒、以及网页打开期间每 30 秒检查一次；打开这个面板只查看、不改数据。</p></div><div class="modal-footer"><span></span><div><button class="secondary danger-button" id="sync-unbind">解除绑定</button><button class="primary" id="sync-now">立即同步</button></div></div></div>`}
if(!dlg.open)dlg.showModal();
const refreshPanel=()=>{if(!dlg.open)return;sync.inspect().then(()=>{if(dlg.querySelector('#sync-now')&&dlg.open)showSync(false)}).catch(e=>toast(e&&e.message?e.message:'无法查看云端'))};
dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>dlg.close());
dlg.querySelector('#gen-code')?.addEventListener('click',()=>{dlg.querySelector('#sync-code').value=sync.generateCode()});
dlg.querySelector('#bind-go')?.addEventListener('click',()=>{try{sync.bindCode(dlg.querySelector('#sync-code').value)}catch(e){toast(e.message);return}dlg.close();toast('同步码已绑定，开始首次同步');sync.tick(true,{allowEmptyPush:true}).then(()=>{renderApp();showSync(false)})});
dlg.querySelector('#sync-now')?.addEventListener('click',()=>{dlg.close();sync.tick(true).then(()=>{renderApp();if(sync.isBound())showSync(false)})});
