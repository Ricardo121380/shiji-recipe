...
if(!bound){dlg.innerHTML=`<div class="editor"><div class="modal-heading"><div><span class="eyebrow">CLOUD SYNC</span><h2>云端同步</h2></div><button class="icon-button" data-close aria-label="关闭">${ico('close')}</button></div><div class="editor-content"><p style="margin-top:0">绑定同步码后，其他设备输入同一个码即可互通。上传或下载由你选择：上传用本机覆盖云端，下载用云端覆盖本机。</p><label class="field">同步码 <span class="optional">建议使用生成的随机码，切勿使用简单词</span><input id="sync-code" maxlength="48" placeholder="例如：fanfun-8f3k2m9x"></label><p class="muted">同步码是数据的唯一凭证，请勿泄露；所有设备的饮食数据将存入 Cloudflare。</p></div><div class="modal-footer"><span></span><div><button class="secondary" id="gen-code">生成随机码</button><button class="primary" id="bind-go">绑定</button></div></div></div>`}
else{dlg.innerHTML=`<div class="editor"><div class="modal-heading"><div><span class="eyebrow">CLOUD SYNC</span><h2>云端同步</h2></div><button class="icon-button" data-close aria-label="关闭">${ico('close')}</button></div><div class="editor-content"><p style="margin-top:0">同步码：<strong>······${sync.getCode().slice(-4)}</strong>（完整码仅存于各设备浏览器）</p><p>本机：<strong>${S.recipes.length}</strong> 道菜谱 · 云端：<strong>${sync.lastCloudRecipes()}</strong> 道菜谱<br>云端最后更新：${sync.lastCloudAt()||'尚无'}<br>本地上次同步：${sync.lastSyncAt()||'尚无'}</p><p class="muted">${esc(sync.lastPlanHint()||'正在查看云端状态…')}</p><label class="prep-line"><input type="checkbox" id="sync-pause" ${sync.isPaused()?'checked':''}> 暂停自动备份（改动只保留在本机）</label><p class="muted">请自己选择方向：<strong>上传</strong>用本机覆盖云端，<strong>下载</strong>用云端覆盖本机，两边不会合并。打开本面板只查看、不改数据。后台自动备份只会在云端没有更新时上传本机改动，不会自动下载覆盖本机。</p></div><div class="modal-footer"><button class="danger-button" id="sync-unbind">${ico('trash',14)} 解除绑定</button><div><button class="secondary" id="sync-pull">下载到本机</button><button class="primary" id="sync-push">上传到云端</button></div></div></div>`}
if(!dlg.open)dlg.showModal();
const refreshPanel=()=>{if(!dlg.open)return;sync.inspect().then(()=>{if(dlg.querySelector('#sync-push')&&dlg.open)showSync(false)}).catch(e=>toast(e&&e.message?e.message:'无法查看云端'))};
dlg.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>dlg.close());
dlg.querySelector('#gen-code')?.addEventListener('click',()=>{dlg.querySelector('#sync-code').value=sync.generateCode()});
dlg.querySelector('#bind-go')?.addEventListener('click',()=>{try{sync.bindCode(dlg.querySelector('#sync-code').value)}catch(e){toast(e.message);return}dlg.close();toast('同步码已绑定');sync.afterBind().then(()=>{renderApp();showSync(false)})});
dlg.querySelector('#sync-push')?.addEventListener('click',()=>{dlg.close();sync.pushNow().then(()=>{renderApp();if(sync.isBound())showSync(false)})});
dlg.querySelector('#sync-pull')?.addEventListener('click',()=>{dlg.close();sync.pullNow().then(()=>{renderApp();if(sync.isBound())showSync(false)})});
